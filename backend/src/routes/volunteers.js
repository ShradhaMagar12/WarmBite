...
router.post('/', wrapAsync(async (req, res) => {
    ...
    await VolunteersService.create(req.body.data, req.currentUser, true, link.host);
    const payload = true;
...
router.post('/deleteByIds', wrapAsync(async (req, res) => {
    await VolunteersService.deleteByIds(req.body.data, req.currentUser);
    const payload = true;
    res.status(200).send(payload);
  }));
...
router.get('/', wrapAsync(async (req, res) => {
  ...
  const payload = await VolunteersDBApi.findAll(
...
router.get('/:id', wrapAsync(async (req, res) => {
  const payload = await VolunteersDBApi.findBy(
    { id: req.params.id },
  );
